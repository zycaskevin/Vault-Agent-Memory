...
def test_contract_is_closed_and_inactive_before_owner_confirmation() -> None:
    ...
    assert contract["authority"] == {
        ...
        "owner_decision_id": "SD-TASK-AUTH-V4-MISSION",
    ...
    assert contract["activation"]["bridge_implementation_base_commit"] == (
        "git:f7aff39fecbc2fce7d612f396237afb0e094e460"
    )
...
def test_required_ci_pins_every_nonrecursive_v4_bridge_root() -> None:
...
def test_revocation_is_monotonic_and_blocks_task_derivation(tmp_path: Path) -> None:
    ...
    revocation = {
        ...
        "authorizing_principal": "github:zycaskevin",
        "owner_confirmation_ref": "owner-message:mission-v4-revocation-test",
    }
    ...
    with pytest.raises(mission.Denied):
        mission.derive_task_authorization(
            ...
            proof,
            "T-004",
            "f7aff39fecbc2fce7d612f396237afb0e094e460",
...
def test_protocol_release_accepts_exact_two_modifications_and_eleven_additions(
    ...
    modified = {".github/workflows/ci.yml", "tests/test_repo_hygiene_tools.py"}
    ...
    with pytest.raises(mission.Denied):
        mission._check_protocol_release_commit(repo, head)
...
def test_source_review_binds_retained_required_reads_outputs_and_proof() -> None:
    ...
    review = {
        "schema_version": 4,
        "artifact_kind": "subject-task-source-review-v4",
        "status": "PASS",
        "authorized_task": "T-020",
        "mission_id": mission_proof["mission_id"],
...
def test_expiry_or_revocation_discards_only_exact_unpublished_transition(
    ...
    review_raw = mission.canonical(review)
    refs = [{"kind": "opaque", "id": "mission-v4:exact"}]
...
def test_start_and_completion_reuse_exact_pending_event_bytes(
    ...
    if current_state == "IN_PROGRESS":
        ...
        current["updated_at_utc"] = "2026-08-13T00:00:01Z"
    refs = [{"kind": "opaque", "id": "mission-v4:test-ref"}]
    pending = copy.deepcopy(current)
    pending["tasks"]["T-004"] = target
    pending["events"].append(
        {
...
def test_preliminary_delivery_binds_exact_git_head_tree_diff_and_required_ci(
    ...
    delivery = {
        "schema_version": 4,
        "artifact_kind": "subject-task-preliminary-delivery-v4",
        "status": "PASS",
