...
ABSOLUTE_PATH = re.compile(
    ...
    rb"|[A-Za-z]:\\[A-Za-z0-9._ -]+(?:\\[A-Za-z0-9._ -]+)*(?![-A-Za-z0-9._/\\])"
    rb"|\\\\[A-Za-z0-9._$ -]+\\[A-Za-z0-9._$ -]+(?:\\[A-Za-z0-9._$ -]+)*(?![-A-Za-z0-9._/\\])"
    rb")"
)
LOCAL_FILE_URI = re.compile(
    rb"(?i)(?<![A-Za-z0-9._-])(?:jar:)*file:/"
)
...
def _public_safety_errors(raw: bytes, document: dict[str, Any]) -> list[str]:
    ...
    if any(
        ABSOLUTE_PATH.search(payload) is not None
        or LOCAL_FILE_URI.search(payload) is not None
        for payload in payloads
    ):
...
def test_public_safety_scan_rejects_each_forbidden_family() -> None:
    ...
    controls = (
        ...
        ({"note": "synthetic output at /tmp"}, "absolute_path"),
        ({"note": "synthetic output at /private"}, "absolute_path"),
        ({"note": "synthetic output at /tmp/private/item"}, "absolute_path"),
        ({"note": "leak (/tmp)"}, "absolute_path"),
        ({"note": "path=/private"}, "absolute_path"),
        ({"note": "leak /tmp/private,"}, "absolute_path"),
        ({"note": "synthetic output at /var/folders/demo/item"}, "absolute_path"),
        ({"note": r"synthetic output at C:\Temp\item"}, "absolute_path"),
        ({"note": r"synthetic output at \\server\share"}, "absolute_path"),
        ({"note": "file:///tmp/private/item"}, "absolute_path"),
        ({"note": "file://localhost/tmp"}, "absolute_path"),
        ({"note": "FILE:///Users/demo"}, "absolute_path"),
        ({"note": "jar:file:///tmp/a"}, "absolute_path"),
...
def test_public_safety_scan_allows_uri_and_embedded_slash_nonpaths() -> None:
    legal_controls = (
        {"note": "https://example.invalid/path"},
        {"note": "synthetic://subject-distillation/e-p-001"},
        {"note": "choose and/or compare"},
        {"note": "version/one"},
    )
    for document in legal_controls:
        raw = json.dumps(document).encode()
        assert _public_safety_errors(raw, document) == []
