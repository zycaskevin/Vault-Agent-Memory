...
- Create: `guardrails_lite/agent_policy.py`
...
- `guardrails_lite/guardrails_health.py`：從本地 SQLite 收集 Document Map health metrics。
- `scripts/sync_to_supabase.py --health` / `--guardrails-health`：把每日 snapshot upsert 到 `hermes_guardrails_health`。
...
| Document Map metric | `hermes_guardrails_health` 欄位 |
|---|---|
...
**踩坑決策：** deployed `hermes_guardrails_health` 沒有 `id` 欄位，不能用 generic `_upsert_by_key(..., key_fields=("id",))`；daily snapshot 必須以 `check_date` 查詢後 update/insert，或用 REST `on_conflict=check_date`。Sprint 4C 採用專用 `_upsert_guardrails_health_by_check_date()`，避免 schema drift。
