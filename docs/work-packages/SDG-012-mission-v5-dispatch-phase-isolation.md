...
## PR #491 review remediation v4

- PR #491 exact head `7ac2f2c6b6e27cea8f488fd261f5f3e05b242846`
  passed Release Readiness run `31959287396`, including Python 3.10 job
  `95194687238`, Python 3.11 job `95194687365`, Python 3.12 job `95194687218`,
  and governance job `95194687188`. Hosted Green is historical evidence for
  v3 only; it does not close later review findings or transfer to v4.
- Independent disposition is P0=0/P1=1. The accepted P1 retains the direct
  malformed-delivery validator DENY and additionally requires dispatcher API
  `Denied` plus CLI rc 2, empty stdout, exact DENY stderr, and no traceback or
  ERROR mapping before rollback mutation.
- CodeRabbit's suggested equality between the SDG-012 delivery topic and PR
  #487's Mission proof fixture is rejected as an architecture mismatch: they
  are deliberately different objects. v4 instead binds the replacement
  delivery to the unique merged PR for exact branch
  `agent/sdg012-identity-junit-dispatcher-only-v4`, and independently pins PR
  #487 as CLOSED on base `9ddc508...`, branch
  `agent/mission-v5-activation-post-sdg011`, exact head `6e596574...`, and no
  merge commit.
- The safe transition-test improvement is accepted: canonical `origin/main`
  is denied at the SDG-011 anchor, accepted only when it is the exact immediate
  SDG-012 two-parent delivery, and denied for a topic or later descendant.
- Hosted timing provides measured headroom rather than a timeout guess. The
  slowest candidate identity step was 187 seconds and the slowest Python job
  was 304 seconds against a 20-minute job timeout, leaving at least 896 seconds
  of observed job-level headroom. Full-history missing-object diagnostics stay
  a final bundle-verification requirement; no production or trust architecture
  is changed for that nonblocking nitpick.
