...
This page positions Vault Agent Memory against common agent-memory systems. It is not a vendor benchmark; it is an operator checklist for choosing the right memory layer for a project.

## Short answer

Vault Agent Memory is strongest when you want a **local-first, inspectable, file-backed memory QA layer** for coding agents or project agents. It emphasizes candidate review, deterministic gates, bounded citations, Search QA snapshots, SQLite backups, and Markdown source control.
...
| System | Primary shape | Best fit | Storage / control | Memory write model | Recall / retrieval model | QA / audit posture | Trade-offs vs Vault Agent Memory |
|---|---|---|---|---|---|---|---|
| **Vault Agent Memory** | Local SQLite + Markdown source + MCP tools | Project/coding-agent memory where operators want inspectability, backups, and regression tests | Local SQLite is source of truth; Markdown `raw/` can be reviewed and versioned | Candidate-first `remember`/`promote`, privacy/duplicate/metadata/quality gates, direct `vault_add` only for trusted compatibility | Keyword-first FTS/BM25 with fallback; optional semantic/hybrid vectors; Document Map for bounded reads | Built-in Search QA snapshots, public PR gate, privacy/history scan, artifact audit, backup/verify/restore | Not a hosted SaaS memory platform; semantic quality depends on configured embedding provider; no enterprise auth layer in local MCP server |
...
- Choose **Vault Agent Memory** when memory should be local, inspectable, backed up, cited from bounded source ranges, and measured with deterministic regression checks.
