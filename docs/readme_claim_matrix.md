...
Scope: public README feature/capability claims after the v0.6.71 automation cycle workspace update. Localized README files should mirror the same maturity and non-goal language.
...
| C04 | Manual quickstart works from PyPI. | stable | `vault-for-llm[mcp]==0.6.71` installs and exposes `vault`. | Clean Python 3.11 PyPI install smoke is part of release closeout. |
...
python scripts/check_release_parity.py --tag v0.6.71
...
For release v0.6.71, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.71` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.71`.
...
- `vault setup-agent --automation-schedule all --automation-include-transcripts` generates cron, LaunchAgent, n8n, and README templates that default to `vault automation cycle` and write metadata-only transcript hints into the inbox handoff.
- `vault automation cycle --write-workspace --include-transcripts` writes a compact `cycle-latest.json` with candidate review, transcript paths, and curation policy summary.
