...
Scope: public README feature/capability claims after the v0.6.52 product README cleanup. Localized README files should mirror the same maturity and non-goal language.
...
| C04 | Manual quickstart works from PyPI. | stable | `vault-for-llm[mcp]==0.6.52` installs and exposes `vault`. | Clean Python 3.11 PyPI install smoke is part of release closeout. |
...
| C09 | Policy-based automation is report-first by default. | usable-alpha | `vault automation plan/run/report/doctor`; reports include a dry-run diff and action ledger, and generated schedule templates omit `--apply` unless explicitly requested. | Automation tests, PyPI setup-agent smoke, and full pytest passed before release. |
...
python scripts/check_release_parity.py --tag v0.6.52
...
For release v0.6.52, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.52` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.52`.
