...
Scope: public README feature/capability claims after the v0.6.61 product README cleanup. Localized README files should mirror the same maturity and non-goal language.
...
| C04 | Manual quickstart works from PyPI. | stable | `vault-for-llm[mcp]==0.6.61` installs and exposes `vault`. | Clean Python 3.11 PyPI install smoke is part of release closeout. |
...
python scripts/check_release_parity.py --tag v0.6.61
...
For release v0.6.61, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.61` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.61`.
- `vault setup-agent --automation-schedule all` generates cron, LaunchAgent, n8n, and README templates that default to `vault automation cycle`.
