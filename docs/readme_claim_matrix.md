...
Scope: public README feature/capability claims after the v0.6.73 scheduled cycle workspace update. Localized README files should mirror the same maturity and non-goal language.
...
| C04 | Manual quickstart works from PyPI. | stable | `vault-for-llm[mcp]==0.6.73` installs and exposes `vault`. | Clean Python 3.11 PyPI install smoke is part of release closeout. |
...
python scripts/check_release_parity.py --tag v0.6.73
...
For release v0.6.73, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.73` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.73`.
...
- `vault automation cycle --write-workspace --include-transcripts` writes compact `cycle-latest.json` and `cycle-latest.md` handoffs with candidate review, transcript paths, and curation policy summary.
