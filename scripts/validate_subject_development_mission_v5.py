#!/usr/bin/env python3
"""Fail-closed validator for the Subject Development Mission v5 root."""
...
try:
    mission = _load_sibling_dependency(
        "scripts.run_subject_development_mission_v5",
    ...
    if __name__ == "__main__":
        sys.stderr.write("SUBJECT_DEVELOPMENT_MISSION_V5_VALIDATION_ERROR\n")
        raise SystemExit(3) from None
    raise


Denied = mission.Denied
DENY_TEXT = "SUBJECT_DEVELOPMENT_MISSION_V5_VALIDATION_DENY\n"
ERROR_TEXT = "SUBJECT_DEVELOPMENT_MISSION_V5_VALIDATION_ERROR\n"
...
def validate_mission_proof_value(
    ...
    *,
    ...
    if (
        ...
        or value["artifact_kind"] != "subject-development-mission-v5-proof"
...
def validate_task_authorization_value(
    ...
    *,
    ...
    if (
        ...
        or raw != mission.canonical(value)
        or value["schema_version"] != 5
        or value["artifact_kind"] != "subject-task-authorization-v5"
        or value["status"] != "PASS"
...
def validate_source_review_value(
    ...
    *,
    ...
    if (
        ...
        or raw != mission.canonical(value)
        or value["schema_version"] != 5
        or value["artifact_kind"] != "subject-task-source-review-v5"
        or value["status"] != "PASS"
...
def validate_preliminary_delivery_value(
    ...
    *,
    ...
    if (
        ...
        or raw != mission.canonical(value)
        or value["schema_version"] != 5
        or value["artifact_kind"] != "subject-task-preliminary-delivery-v5"
        or value["status"] != "PASS"
...
def build_completion_review(
    ...
    value = {
        "schema_version": 5,
        "artifact_kind": "subject-task-completion-review-v5",
        "status": "PASS",
...
def validate_completion_review_value(
    ...
    *,
    ...
    if (
        ...
        or raw != mission.canonical(value)
        or value["schema_version"] != 5
        or value["artifact_kind"] != "subject-task-completion-review-v5"
        or value["status"] != "PASS"
        or value["authorized_task"] != task
    ):
        raise Denied
    ...
    for index, event in enumerate(value["events"][6:], start=6):
        ...
        if event["from"] == "IN_PROGRESS" and event["to"] == "COMPLETED":
            ...
            if task == "T-033":
                ...
                # and private/experimental final gate.  V5 only proves that the
