#!/usr/bin/env python3
"""Owner-confirmed additive authorization bridge for Subject T-002+ tasks."""
...
DENY_TEXT = "SUBJECT_TASK_AUTHORIZATION_V2_DENY\n"
ERROR_TEXT = "SUBJECT_TASK_AUTHORIZATION_V2_ERROR\n"
CLEANUP_REQUIRED = '{"status":"private_cleanup_required"}\n'
AUTHORITY = "github:zycaskevin"
PROTOCOL_DECISION_ID = "SD-TASK-AUTH-V2-BRIDGE"
PROPOSAL_KIND = "subject-task-authorization-v2-proposal"
PROOF_KIND = "subject-task-implementation-authorization-proof"
CONTRACT_PATH = "specs/subject-distillation/task-authorization-v2.contract.json"
PROOF_SCHEMA_PATH = "specs/subject-distillation/task-authorization-v2.schema.json"
...
VALIDATOR_PATH = "scripts/validate_subject_task_authorization_v2.py"
UPDATER_PATH = "scripts/update_subject_task_progress_v2.py"
...
V2_DIGEST_KEYS = {
    ...
    "authorization_runner_v2_sha256",
    "progress_sha256",
    ...
    "task_progress_updater_v2_sha256",
}
...
def _scan_v2(value: Any, owning_key: str | None = None) -> None:
    if type(value) is dict:
        ...
        if normalized == "implementation_base_commit":
            if COMMIT.fullmatch(value) is None and re.fullmatch(
                ...
                raise Denied
            return
        if normalized in V2_DIGEST_KEYS:
...
def _validate_contract(value: Any, raw: bytes) -> dict[str, Any]:
    ...
    if (
        ...
        or value["artifact_kind"] != "subject-task-authorization-v2-contract"
        or value["repository"] != "zycaskevin/Vault-Agent-Memory"
        or value["allowed_tasks"] != {"first": "T-002", "last": "T-002"}
    ...
    if value["authority"] != {
        ...
        "owner_decision_ref": "owner-message:SD-TASK-AUTH-V2-BRIDGE",
    }:
        raise Denied
    ...
    if activation != {
        ...
        "implementation_base_commit": "git:ec841f32c0e735ba0fe027552ed712b1ecb2a440",
        "progress": {
            "path": PROGRESS_PATH,
            "sha256": "ab723c1adde2739f54deba7fee85d86a95002f167703354695503363154d30e6",
        },
        ...
        raise Denied
    if value["trust_root"] != [
        {"path": V1_RUNNER_PATH, "sha256": EXPECTED_V1_RUNNER_SHA256},
        {"path": PROGRESS_VALIDATOR_PATH, "sha256": EXPECTED_PROGRESS_VALIDATOR_SHA256},
        {
            "path": "scripts/verify_subject_implementation_authorization.py",
            "sha256": EXPECTED_VERIFIER_SHA256,
        },
        {
            "path": "specs/subject-distillation/evidence-schemas/implementation-authorization.schema.json",
            "sha256": EXPECTED_V1_SCHEMA_SHA256,
        },
    ]:
        raise Denied
    return value
...
def _validate_scope_descriptor(value: Any, raw: bytes, task: str) -> dict[str, Any]:
    if type(value) is not dict or set(value) != DESCRIPTOR_KEYS:
        raise Denied
    if raw != _canonical(value) or value["schema_version"] != 2:
        raise Denied
    ...
    for policy in policies:
        ...
        if policy["action"] not in {"create", "modify", "transient"}:
            raise Denied
        expected_mode = "absent" if policy["action"] == "transient" else "0644"
    ...
    if any(v1.verifier.NON_GOAL.fullmatch(item) is None for item in non_goals):
        raise Denied
    argv = value["verification_argv"]
    if (
        type(argv) is not list
        or not 1 <= len(argv) <= 32
        or any(type(item) is not str or not 1 <= len(item) <= 256 for item in argv)
    ):
        raise Denied
    try:
        _scan_v2(value)
...
def _support_hashes(repo_root: Path) -> dict[str, str]:
    paths = {
        ...
        "task_progress_updater_v2_sha256": UPDATER_PATH,
    ...
    result["authorization_runner_v2_sha256"] = hashlib.sha256(self_raw).hexdigest()
    return result
...
def _derive(
    ...
    proposal = {
        ...
        "authorization_runner_v2_sha256": support["authorization_runner_v2_sha256"],
        ...
        "task_progress_updater_v2_sha256": support["task_progress_updater_v2_sha256"],
...
def _parse_proposal(raw_text: str, now: datetime) -> tuple[dict[str, Any], datetime]:
    ...
    if type(value) is not dict or set(value) != PROPOSAL_KEYS or raw != _canonical(value):
        raise Denied
    if value["schema_version"] != 2 or value["artifact_kind"] != PROPOSAL_KIND:
        raise Denied
...
def _proof(proposal: dict[str, Any], recorded: datetime, owner_ref: str) -> dict[str, Any]:
    ...
    return {
        ...
        "authorization_runner_v2_sha256": proposal["authorization_runner_v2_sha256"],
        ...
        "task_progress_updater_v2_sha256": proposal["task_progress_updater_v2_sha256"],
...
def _verify_confirmed(values: dict[str, str], runtime: Runtime) -> bytes:
    ...
    with v1._authorization_lock(state.repo_root, "task-auth-v2", base, v1_runtime) as audit_lock:
        ...
        if failure is not None:
            raise failure
        if _progress(runtime, root, task) != initial_progress or _repository_state(runtime) != state:
            raise Denied
        recorded = _clock(runtime).replace(microsecond=0)
        _require_recorded_freshness(proposal, issued, recorded)
        proof = _proof(proposal, recorded, owner_ref)
