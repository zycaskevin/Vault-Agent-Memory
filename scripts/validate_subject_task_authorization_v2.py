...
DENY_TEXT = "SUBJECT_TASK_AUTHORIZATION_V2_VALIDATOR_DENY\n"
ERROR_TEXT = "SUBJECT_TASK_AUTHORIZATION_V2_VALIDATOR_ERROR\n"
    ...
    *,
    ...
    if retained is None:
        ...
        support_paths = {
            ...
            "task_progress_updater_v2_sha256": runner.UPDATER_PATH,
            "authorization_runner_v1_sha256": runner.V1_RUNNER_PATH,
            "progress_validator_sha256": runner.PROGRESS_VALIDATOR_PATH,
            "authorization_runner_v2_sha256": "scripts/run_subject_task_authorization_v2.py",
        }
        try:
            support = {
...
def _reviewed_outputs(
    ...
    for path in descriptor["completion_repo_relative_paths"]:
        ...
        runner._require_public_identity(identity, 0o644)
        outputs.append(
            {"mode": "100644", "path": path, "sha256": hashlib.sha256(raw).hexdigest()}
        )
    return outputs
...
def validate_completion_review_value(
    ...
    *,
    ...
    if (
        value["schema_version"] != 2
        or value["artifact_kind"] != "subject-task-completion-review-v2"
        or value["status"] != "PASS"
    ...
    try:
        ...
        runner._scan_v2(progress_value)
    except (runner.v1.verifier.Denied, runner.Denied):
        raise Denied from None
    ...
    if (
        ...
        or value["verification_argv"] != descriptor["verification_argv"]
    ...
    for item in changes:
        if (
            ...
            or set(item) != {"action", "mode", "path", "sha256"}
            or item["action"] not in {"add", "modify"}
            or item["mode"] != "100644"
        ...
        if retained is None:
            ...
            runner._require_public_identity(change_identity, 0o644)
        else:
            try:
...
def validate_completion_review(path: Path, task: str = "T-002") -> dict[str, Any]:
...
def validate_ledger_value(
    ...
    *,
    ...
    _require_supported_task(repo_root, "T-002", retained=retained)
    if type(value) is not dict or type(value.get("events")) is not list:
        raise Denied
    proofs = 0
    ...
    for index, event in enumerate(value["events"]):
        if type(event) is not dict or event.get("task_id") == "T-001":
            continue
        task = _require_supported_task(
...
def validate_ledger(repo_root: Path) -> dict[str, Any]:
    base_guard = runner._open_bridge_guard(
        repo_root, "T-002", include_progress=True, include_proof=False
    )
    ...
    try:
        ...
        started = any(
            event.get("task_id") == "T-002" and event.get("to") == "IN_PROGRESS"
            for event in value["events"]
            if type(event) is dict
        )
        review_name = "T-002.review.json"
        review_present = review_name in names_before
        if started:
            descriptor_raw = retained[runner._scope_path("T-002")]
            descriptor_value = runner.v1.verifier._parse(descriptor_raw)
            descriptor = runner._validate_scope_descriptor(
                descriptor_value, descriptor_raw, "T-002"
            )
            extras: list[str] = []
            if review_present or value["tasks"]["T-002"] == "COMPLETED":
                extras.extend(descriptor["completion_repo_relative_paths"])
            if review_present:
                extras.append(_review_path("T-002"))
        ...
        expected = ["README.md"] + (["T-002.json"] if started else [])
        if review_present:
            expected.append(review_name)
        if value["tasks"]["T-002"] == "COMPLETED" and not review_present:
            raise Denied
        if names_before != sorted(expected):
            raise Denied
        if value["tasks"]["T-002"] == "COMPLETED":
            proof_raw = retained[
                "specs/subject-distillation/task-authorizations/T-002.json"
            ]
            proof_value = runner.v1.verifier._parse(proof_raw)
            review_raw = retained[_review_path("T-002")]
            ...
            review_change = next(
                (item for item in changes if item["path"] == _review_path("T-002")),
                None,
            )
            source_changes = [
                item
                for item in changes
                if item["path"] not in {_review_path("T-002"), runner.PROGRESS_PATH}
            ]
