#!/usr/bin/env python3
"""Proof-aware atomic start/completion wrapper for Subject authorization v3."""
...
try:
    ...
    import validate_subject_task_authorization_v3 as validator
    ...
    try:
        ...
        from scripts import validate_subject_task_authorization_v3 as validator
    except Exception:  # noqa: BLE001 - fixed startup boundary
        runner = writer_v1 = validator = None
...
DENY_TEXT = "SUBJECT_TASK_PROGRESS_V3_DENY\n"
ERROR_TEXT = "SUBJECT_TASK_PROGRESS_V3_ERROR\n"
if _BOOTSTRAP_ERROR:
    ...
    *,
    ...
    try:
        with runner.v1._authorization_lock(
            ...
            if names not in (
                ["README.md", "T-002.json", "T-002.review.json", "T-003.json"],
                [
                    "README.md",
                    "T-002.json",
                    "T-002.review.json",
                ...
                raise Denied
            review_present = "T-003.review.json" in names
            review_pending_present = runner._repo_entry_exists(
                repo_root, runner.PROOF_PENDING_PATH
            )
            current_raw = prestate.snapshot()[runner.PROGRESS_PATH]
            try:
                current = runner.v1.verifier._parse(current_raw)
                runner._scan_v3(current)
            except (runner.v1.verifier.Denied, runner.Denied):
                raise Denied from None
            ...
            if not review_present and not review_pending_present:
                changes_before = runner._repository_changes(
                    repo_root,
                    review_value["implementation_base_commit"],
                    review_value["reviewed_change_paths"] + [runner.PROGRESS_PATH],
                    retained,
                )
                if (
                    [
