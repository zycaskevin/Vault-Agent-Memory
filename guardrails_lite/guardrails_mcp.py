...
使用方式：
  1. 作為 stdio MCP server（給 agent runtime config.yaml 用）：
     python3 vault/guardrails_mcp.py

  2. 在 agent runtime config.yaml 加入：
     mcp_servers:
       guardrails:
         command: "conda"
         args: ["run", "-n", "guardrails-lite", "python3", "/home/user/Guardrails-knowledge/vault/guardrails_mcp.py"]
"""
