...
class GuardrailsDB:
    ...
    def _init_vec_table(self):
        ...
        try:
            ...
            if "already exists" in str(e).lower() or "different" in str(e).lower():
                log.warning(f"⚠️ 向量表維度不匹配，重建中（舊向量會遺失）: {e}")
        ...
        for from_ver in range(current, self.SCHEMA_VERSION):
            ...
            for sql, desc in steps:
                try:
                    ...
                    self.conn.commit()
                    log.info(f"✅ migration v{from_ver}→v{from_ver+1}: {desc}")
                except Exception as e:
                    # 欄位已存在（重複執行）→ 忽略
                    if "duplicate column" in str(e).lower() or "already exists" in str(e).lower():
                        pass
                    else:
                        log.warning(f"⚠️ migration 失敗: {desc}: {e}")
    ...
    def _search_fts5(
        ...
        try:
            ...
            # FTS5 查詢失敗 → 靜默降級
            log.warning(f"⚠️ FTS5 搜尋失敗，降級 LIKE: {e}")
            return None
