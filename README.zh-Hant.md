...
[官方網站](https://zycaskevin.github.io/Vault-Agent-Memory/) · [Founding 10 挑戰](https://zycaskevin.github.io/Vault-Agent-Memory/challenge/) · [公開測試](https://zycaskevin.github.io/Vault-Agent-Memory/benchmarks/) · [第三方獨立重跑](https://zycaskevin.github.io/Vault-Agent-Memory/reproduce/) · [測試方法](https://zycaskevin.github.io/Vault-Agent-Memory/benchmarks/methodology/) · [安裝](#安裝)
...
```text
幫這個專案安裝 Vault Agent Memory。使用 vault-for-llm[mcp]==0.10.2。
...
Vault 比較像補在既有 Agent 之間的「記憶治理層」，不是要取代所有記憶服務或
Agent runtime。

| 工具類型 | 強項 | Vault 不同的重點 |
|----------|------|------------------|
| [Mem0](https://docs.mem0.ai/platform/overview) | Managed memory API、個人化、cloud/self-hosted 部署、公開記憶 benchmark | Local-first governance、審核關卡、rollback、每日人工 review、多 Agent adapter setup |
| [Letta / MemGPT](https://docs.letta.com/letta-agent) | Stateful agent platform、memory-first agent、dreaming、skills、Agent 自主管理記憶 | 讓既有 Codex、OpenClaw、Claude、Coze、n8n 等 Agent 共用同一個受治理記憶層 |
| Vector DB / RAG stack | 快速查 embeddings 和文件 | 管理 retrieval 前後的生命週期：候選、信任度、敏感度、時效、來源範圍、audit |
| 一般筆記 / Obsidian | 人類閱讀與知識整理 | Agent-safe access、bounded read、治理 metadata、候選提升、可選 Obsidian sync |

如果你要的是 app 內可快速接上的 managed memory API，Mem0 可能更快。如果你要的是
...
衡量的是 end-to-end memory QA，只有在同一套 harness、同等條件跑過後才適合公開比較。
...
| Layer | 用途 |
|---|---|
| `L0` | 穩定啟動基線與專案定位（`L0-bootstrap`） |
| `L1` | 穩定事實、規則、偏好 |
| `L2` | 已審查的近期上下文、摘要、短中期背景 |
| `L3` | 詳細知識、SOP、bug、決策、來源筆記 |

新專案建立 `L0-bootstrap`。既有 `L0-identity` 路徑仍可讀，且不會被自動
重新命名。L0 是啟動／專案上下文，不是 Vault 擁有的身份或人類模型。
