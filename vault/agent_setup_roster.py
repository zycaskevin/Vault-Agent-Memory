        ...
        path.write_text(
            "\n".join(
                [
                    ...
                    "3. Fail closed if the imported OpenAPI `servers.url` still contains `YOUR_PROJECT`, `vault.example.internal`, or another placeholder.",
                    "4. Configure the Supabase anon key as both the `apikey` header and the bearer value for the authorization header.",
                    "5. Call `vaultRemoteSearch` with `p_agent_id`, `p_query`, `p_include_private=false`, `p_max_sensitivity=medium`, and `p_limit=5`.",
                    "6. Confirm responses contain safe summaries and do not expose `content_raw`.",
                    "7. Run `vault remote status --json` and require `remote_reader_openapi_placeholder_url` to be absent before calling Coze connected.",
                    "",
                ]
            ),
            encoding="utf-8",
