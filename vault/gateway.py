...
def gateway_memory_timeline(
    ...
    *,
    ...
    policy_probe = gateway_read_range(
        project,
        knowledge_id=int(memory_id),
        agent_id=agent,
        line_start=1,
    ...
    class VaultGatewayHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
            ...
            if parsed.path == "/memory/audit":
                payload = gateway_memory_audit(
                    project,
                    agent_id=_request_agent({}, parsed),
        ...
        def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
            ...
            if parsed.path == "/memory/search":
                ...
                _append_audit(project, "memory_api_search", agent, payload.get("status", "ok"), query=str(body.get("query", "")), **self._audit_context(parsed))
                self._send_json(payload)
                return
...
def _str_query(parsed, name: str, default: str) -> str:
    query = parse_qs(parsed.query)
    return str((query.get(name) or [default])[0] or default)


def _int_query(parsed, name: str, default: int) -> int:
    return _int_value(_str_query(parsed, name, str(default)), default)


def _optional_int_query(parsed, name: str) -> int | None:
    query = parse_qs(parsed.query)
...
def _bool_query(parsed, name: str, default: bool) -> bool:
    query = parse_qs(parsed.query)
    if name not in query:
        return bool(default)
