...
compatibility commit is part of that merge. It then creates an uncommitted
revert candidate for `ec107d1` and restores the delivery DEP so no evidence is
deleted. Before committing, replace the restored pre-extraction sentence with
an owner-approved compatibility sentence that retains the exact case-sensitive
phrase `Runtime is not implemented`.
