...
Headroom context 压缩，最后跑 search/read/propose smoke test。
```
...
| 功能 | 默认 | 安装命令 | 什么时候问 |
|---|---|---|---|
| `core` | 是 | `python -m pip install vault-for-llm==0.6.28` | 永远需要：本地 Markdown、SQLite、keyword search。 |
| `mcp` | MCP-capable agent 建议开 | `python -m pip install "vault-for-llm[mcp]==0.6.28"` | runtime 可以接 local stdio MCP tools。 |
...
当前 source tree 已包含 v0.6.28 的 agent integration、OpenClaw adapter、Obsidian 导入同步、benchmark proof 与质量 gate，并保留候选制记忆 workflow 与搜索增强。白话说，Vault 现在不像一个谁都能乱塞纸条的抽屉，更像一间有前台的小图书馆：
...
# 或让 setup-agent 生成每日 cron、LaunchAgent、n8n 模板
vault setup-agent \
  --non-interactive \
  --agent nancy \
  --scope shared \
