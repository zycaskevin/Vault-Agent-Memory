...
更大的 roadmap 是：Vault-for-LLM 是记忆核心，Hermes Agent、OpenClaw、
Claude Code、Codex、n8n、机器人、车上助理、智能家居 Agent 是手，模型是
运算工具。详见 [vision document](docs/vision.md)。

它也不是只为文件设计，而是以使用者为中心设计。Vault 可以保存项目知识，也可以承载经过审核的使用者侧写：稳定工作偏好、沟通边界、近期状态摘要、长期互动模式，让 Agent 不用每个 session 都像第一天认识你。这些侧写记忆仍然要被治理：原始私密互动留在私有层，可信任 Agent 只共享短版、审核过的摘要。

更自动化的用法，是替使用者设计 1-2 个专属记忆 Agent：

- **Profile agent**：维护使用者稳定侧写、偏好、照顾摘要与不同 Agent 的互动边界，不暴露原始私密聊天。
- **Dream / forgetting agent**：定期做 dream report、去重、标记过期、建议 promote/archive，让数据库会整理，也会忘掉低价值或已过期的上下文。

这让 Vault 不只适合今天的聊天 Agent。未来如果要接具身 Agent、长期助理，或更泛用的 world-model workflow，同一套治理结构也能同时保存使用者脉络、项目状态、有来源的知识，以及安全的记忆淡忘。

核心设计原则是 **Progressive Memory Disclosure**：Agent 先看到小而安全的摘要，再逐层打开 topic map、搜索候选、定界来源段落；只有任务和权限真的需要时，才读 raw 或 archived memory。这样记忆金库长大后仍然能保持高效、可审查、可回溯。
...
如果想看它和 PageIndex / Headroom 这类相邻系统的关系，请看
[PageIndex and Headroom comparison](docs/comparisons/pageindex_headroom.md)。
白话版：Vault 可以借鉴 PageIndex 的文件树导航，也可以选择性搭配
Headroom 的 context budget，但核心仍是本地、可治理、可引用的项目记忆。
...
- **治理式读取**：共用 vault 的 Agent 可传 `agent_id` 和敏感度上限，先过滤 private/restricted memory 再做定界读取。
...
| 系统 | 使用方式 |
|---|---|
| Hermes Agent | 设置 `vault-mcp`，让 Agent 使用 search/read/propose tools；用 CLI 跑 dream report、backup、onboarding benchmark。 |
...
- [`docs/agent_install.md`](docs/agent_install.md)：给 Hermes、Codex、Claude Code、OpenClaw、OpenCode、n8n 和其他 Agent 的短版安装 runbook。
...
选到 optional features 后问我要不要现在安装对应 dependencies，
如果选 semantic，问我要不要下载本地 ONNX embedding model，
问我有没有既有 Obsidian vault 要导入，设置 CLI/MCP，需要时做第一次
Obsidian 导入，再问我要不要开启后续自动同步，询问长 tool output 是否需要
Headroom context 压缩，问我要不要生成 Profile / Dream / Forgetting
memory-agent 指引，最后跑 search/read/propose smoke test。
```
...
| 功能 | 默认 | 安装命令 | 什么时候问 |
|---|---|---|---|
| `core` | 是 | `python -m pip install vault-for-llm==0.6.51` | 永远需要：本地 Markdown、SQLite、keyword search。 |
| `mcp` | MCP-capable agent 建议开 | `python -m pip install "vault-for-llm[mcp]==0.6.51"` | runtime 可以接 local stdio MCP tools。 |
...
| `headroom` | 否 | `python -m pip install headroom-ai` | Agent 常读很长的 logs、terminal output 或大量检索内容，需要送进 LLM 前先做可选压缩。 |
| `memory_agents` | 否 | 不需额外依赖 | 用户想启用 Profile / Dream / Forgetting agent 指引，默认 report-only / candidate-only。 |
| `dev` | 否 | `python -m pip install -e ".[dev]"` | source checkout、benchmark、PR 或 release validation。 |

选好 optional features 后，`vault setup-agent` 可以直接替 Agent 安装对应的
Python 依赖。互动模式会先询问是否现在安装；非互动模式请明确加
`--install-optional-deps`。如果选了 semantic，也可以加
`--install-embedding-model mix`，下载并设置默认的本地 ONNX embedding model。
...
不要偷偷开启 semantic、Supabase 或 Headroom extras；semantic 和 Supabase 会增加较重的依赖、模型/provider 设置，或远程凭证；Headroom 只有在真的有 context window 或 token 压力时才需要。若开启 Headroom，正式引用仍要回到原始 `vault_read_range` 内容，不要引用压缩摘要。
...
`/tmp/...` 是一次性测试工作目录，不是包真正安装的位置，也不适合拿来当长期共享记忆库。正式共用时，请选稳定路径，例如 `~/Vaults/my-project`，并让每个可信 Agent 都指向同一个 `project-dir`。若要跑排程工作，Python virtualenv 也应放在稳定路径，例如 `~/.hermes/venvs/vault-for-llm/`；放在 `/tmp/...` 的 venv 重启后可能消失。
...
当前 source tree 已包含 v0.6.51 的 agent integration、OpenClaw adapter、Obsidian 导入同步、benchmark proof 与质量 gate，并保留候选制记忆 workflow 与搜索增强。白话说，Vault 现在不像一个谁都能乱塞纸条的抽屉，更像一间有前台的小图书馆：
...
| Embedding | 可选 ONNX Runtime、Ollama、OpenAI、Cohere、Voyage provider guard 与 durable cache workflow |
...
| 未来检索层 | Document Map 树状导航与 Headroom context-budget 集成设计（[tree navigation](docs/design/document_tree_navigation.md)、[Headroom notes](docs/integrations/headroom.md)） |
...
L0-L3 描述的是记忆深度与用途，不是权限本身。多 Agent 安装时，建议保留
L0-L3 的稳定骨架，再用 `scope`、`sensitivity`、`owner_agent`、
`allowed_agents`、`status`、`memory_type`、`expires_at` 这类 metadata
...
若我选了 optional features，询问是否现在安装对应 dependencies。
若我选了 semantic，询问是否下载本地 ONNX embedding model。
...
也可以使用远程 API provider；API key 只放环境变量，不写进 vault.db：
...
同一路径也支持 `cohere` / `voyage`。完整 provider 与维度设置请看
...
`vault candidates` 会列出待审记忆，默认不倾倒完整原文。MCP agent
应使用 `vault_memory_propose`、`vault_memory_candidates` 和 `vault_memory_promote`；详见 [MCP 记忆流程](docs/mcp_memory_workflow.md)。
...
| `vault remote search "query" --agent-id remote-agent --json` | 搜索 Supabase read-only 远端记忆视图 |
...
| `vault automation plan/run/report/doctor` | Policy-based 记忆维护自动化，默认 report-first，可按 policy 执行可逆归档 |
...
features（MCP、semantic、Supabase、Headroom、dev）、安装语言、是否立即安装已选功能的 dependencies、既有 Obsidian vault 路径、是否做第一次导入，以及是否生成 cron、LaunchAgent 或 n8n 自动同步模板。若选 semantic 并确认安装依赖，精灵也可以下载并设置本地 ONNX embedding model。
若选 Supabase，精灵也可以生成 shell、n8n、Coze remote reader 模板，让 hosted agent 使用 `vault remote smoke/search/map/read` 或 Supabase RPC 读取安全摘要。
...
跨主机或 hosted agent 使用 `remote` profile 时，建议走
`vault_remote_search` → `vault_remote_map_show` → `vault_remote_read_range`。
`vault_remote_search` 会调用 [`docs/supabase_read_policy.sql`](docs/supabase_read_policy.sql)
提供的 `vault_search_readable` RPC，只返回安全 metadata 与摘要，不把 service role key
或原始全文交给一般 Agent。
...
默认先走 simple sync。RLS、多 Agent allow-list、Coze read-only access 属于 advanced setup；需要时再看 [`docs/supabase_setup.md`](docs/supabase_setup.md)。
...
# 套用 docs/supabase_read_policy.sql 后，hosted reader 可用 CLI 远端读取
vault remote smoke --agent-id remote-agent --query "部署 SOP" --json
vault remote search "部署 SOP" --agent-id remote-agent --json
vault remote map 12 --compact --json
vault remote read 12 --node-uid node_install --json

# 或让 setup-agent 生成每日同步与 shell/n8n/Coze remote-reader 模板
vault setup-agent \
  --non-interactive \
  --agent profile-agent \
  --scope shared \
...
| Probe | 结果 | 注意事项 |
|---|---:|---|
| Repo onboarding fixture | Vault top-k/source/read-range guidance `28/28`；Codex transcript baseline `7/28`；Hermes profile transcript baseline `3/28` | 28 题 source-aware project benchmark；private transcripts 不提交进 repo |
