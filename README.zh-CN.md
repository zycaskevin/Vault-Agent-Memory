...
```text
帮这个项目安装 Vault Agent Memory。使用 vault-for-llm[mcp]==0.7.29。
...
SQLite 仍然是最简单的 source of truth。远端共享时，先选 adapter，不要急着把本地数据库变成黑盒云端状态。

Supabase 适合不同主机、n8n、Coze 或 hosted Agent 读取经过筛选的共享记忆：
