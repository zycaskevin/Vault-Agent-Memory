...
Vault-for-LLM is a local-first memory governance layer for agents. It is not
just another RAG database: it shows how shared agent memory moves through a
...
It simulates Codex, Claude Code, and Hermes sharing one Vault:
...
- **Skill registry handoff** - agents can inspect Skill versions, sync manifests,
  and permission-gated registry writes without silently overwriting runtime
  Skill files.
...
`vault automation handoff` automatically surfaces this panel, the latest
review-summary cards, and the latest learning-health dashboard before the
individual cycle/inbox handoff when those files exist.
...
Skill Registry MCP tools follow the same pattern: `review` can inspect Skill
versions and sync manifests, while `maintenance` can write to the local registry
with explicit permission flags. These tools do not install or overwrite runtime
Skill files.
